...
让我们再回顾一下ZE2's argument info结构。每一个arg info结构的声明都是通过`ZEND_BEGIN_ARG_INFO()`或者`ZEND_BEGIN_ARG_INFO_EX()`宏函数开始的，然后紧跟着几行`ZEND_ARG_*INFO()`宏函数，最终以`ZEND_END_ARG_INFO()`宏函数结束。每个宏的基本作用我们可以在第6章的最后一节看到。如果我们想重写一下PHP语言中的`count()`函数，可以：
