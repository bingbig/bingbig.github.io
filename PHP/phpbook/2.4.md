...
我们现在打开Zend/zend_globals.h文件，看一下`_zend_execution_globals`结构体，会在其中发现这么两个element：
...
$GLOBALS不过是`EG(symbol_table)`的一层封装而已。

与之对应，下面的`active_symbol_table`元素也可以通过`EG(active_symbol_table)`的方法来访问，它代表的是处于当前作用域的变量符号表。
